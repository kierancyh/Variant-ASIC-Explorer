...
module tb_rrns_all_lane_top_mrc;
    ...
    initial begin
        $dumpfile("all_lane_mrc_rtl_precheck.vcd");
        $dumpvars(0, tb_rrns_all_lane_top_mrc);
    end
    ...
    begin
        pulse_start(op, a, b);
        wait_done_or_timeout(name, 80, timed_out);
        if (!timed_out) begin
            #1;
    ...
    begin
        $display("%s", banner);
        $display("[MRC RTL] SUMMARY | total=%0d failed=%0d passed=%0d", tests_run, tests_failed, tests_run - tests_failed);
        #20;
        if (tests_failed != 0)
            $fatal(1, "MRC RTL bench had %0d failing tests", tests_failed);
        else
            $finish;
    end
    endtask
    ...
    initial begin
        ...
        finish_report("MRC RRNS correction-stage tests completed.");
    end
