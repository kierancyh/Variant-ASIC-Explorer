...
module tb_final_alu_top;
    ...
    begin
        $display("%s", banner);
        $display("[FINAL ALU RTL] SUMMARY | total=%0d failed=%0d passed=%0d", tests_run, tests_failed, tests_run - tests_failed);
        #20;
        if (tests_failed != 0)
            $fatal(1, "FINAL ALU RTL bench had %0d failing tests", tests_failed);
        else
            $finish;
    end
    endtask
    ...
    initial begin
        ...
        finish_report("FINAL ALU RRNS correction-stage tests completed.");
    end
